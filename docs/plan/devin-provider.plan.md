...
> Status: approved implementation blueprint; implementation complete — Chunks 1–12 are complete, C12.6 is checked, and no next implementation item remains; the final sequential commit is pending parent commit of the implementation diff.
>
> Current next item: none — all Devin tracker items C1.1–C12.6 are checked. C12.1–C12.6 are complete: focused union plus populated v4→v5 migration, `make check`, full race, and tagged smoke gates are green; the smoke harness completes local xAI device and Devin callback OAuth, exercises the exact 5-model×3-protocol×2-mode dispatch ledger, restart/reopen with cross-restart Responses continuation, admin/Web/CLI/worker projections, and a network-isolated populated prior-binary v4 backup restore; final security fixes enforce a required, classified, strict-metadata/error-grammar Connect EndStream envelope and exact callback collision matching; the final review-fix record covers cache-read migration 007, CLI consumed-expiry, Web URL sweeper, strict Connect EndStream classification/metadata, callback matcher, xAI key-rotation fixture, and the CLEAN final split reviews; the ultimate gates are `make check` green, `go test -race ./...` 31 packages ok/2 no tests, tagged smoke race 2 packages ok, and `git diff --check` clean. C12.5 records the exact live-provider disposition: live Devin OAuth/inference was not attempted because operator approval was not provided, credential availability was not evaluated (no claim of unavailable), deterministic local fixtures are not live evidence, the live-only blocker is retained as evidence rather than an open C12.6 dependency, and C12.5 does not satisfy or close the four separately open `init.todo.md` blockers. The final sequential commit `test(provider): complete devin integration acceptance` is pending parent commit of the implementation diff; no commit is made by the tracker.
>
...
Implement compressed Connect request frames, bounded response-frame decoding, trailer/error handling, canonical event mapping, actual model, stop reasons, context cancellation, and per-response usage. Reuse shared non-stream buffering and stream commitment rules. A successful finalize requires a valid Connect EndStream envelope: a top-level JSON object that omits the error key on success; a present error key is parsed as a Connect error object and mapped to a typed provider.UpstreamError reusing the existing HTTP-status classification so routing can classify, cool down, and fail over before the first event (recognized codes: unavailable and internal → transient retry, resource_exhausted → rate-limit, unauthenticated → auth/relogin, permission_denied → permission, invalid_argument → validation; any other valid code → generic upstream). Structurally invalid errors (null, non-object, missing/empty/null/non-string code, invalid message or details field type, or any invalid details element — each details element must be a JSON object with a non-empty string type and a standard-base64 string value) are rejected as malformed. Trailer metadata is exactly map[string][]string (arrays of UTF-8 strings); scalar, null, nested-object, and non-string-array values are rejected, and valid metadata is preserved when present. Each metadata string element is strictly validated against the raw JSON bytes before encoding/json replacement so raw invalid UTF-8 and unpaired or malformed UTF-16 surrogate escapes are rejected as malformed rather than silently substituted with U+FFFD. Raw EOF is always truncation, never a clean success: a zero-byte fresh EOF and EOF after one or more complete data frames both return a sanitized upstream protocol error because the required terminal EndStream envelope never arrived. EOF after 1–4 header bytes or before the full declared payload is read is likewise truncation. Malformed end-stream JSON remains an explicit intentional hardening and is rejected. Finalize runs exactly once, only after a valid success envelope.

Verification: byte-level framing tests, raw/gzip multi-frame streams, delta/cumulative tool arguments, raw-EOF truncation (zero-byte fresh EOF and EOF after complete data frames, since a valid EndStream envelope is required for success), partial-header truncation, partial-payload truncation, EndStream envelope matrix (top-level object required, success omits the error key, valid Connect error objects classified by code into typed provider.UpstreamError, structurally invalid errors (including invalid details elements) and malformed map[string][]string metadata (including invalid UTF-8 and unpaired/malformed surrogate escapes) rejected, valid metadata preserved), malformed-trailer/error cases, first-event flush, pre/post-commit failures, and local input/output/cache-read accounting.
