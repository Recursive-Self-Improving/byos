...
>
> Status: **Chunks 1–12 complete; C12.6 checked; no next implementation item**. Chunk 10 adds provider-aware Admin REST OAuth routes (`POST /admin/api/v1/oauth/devin/start`, `GET /admin/api/v1/oauth/devin/status/{sessionID}`, `POST /admin/api/v1/oauth/devin/cancel/{sessionID}`, plus the exact configured GET callback), provider-typed admin account/model/usage projections with capability-aware refresh/relogin, provider-selectable Web UI (`/admin/oauth/new` chooses `xai|devin`) with correct device versus callback instructions/status, and `byos login --provider xai|devin` CLI with a bounded callback-only HTTP listener reusing the same persisted start/status service. Devin status/cancel are OAuth-session-ID keyed and isolated across concurrent sessions; only the exact callback route bypasses admin bearer auth; capability projections carry only account ID, provider, and enabled state with no credential/raw token/identity/OAuth payload. The sequential Chunk 10 commit is not yet created until the parent commits the implementation diff. Chunk 11 documents deployment, inventory, and attribution: the README/config/route inventory is exact; Compose binds `127.0.0.1` and mounts `deploy/compose.yaml` read-only with the config command; the Railway baked-config workflow is documented; fixed endpoints/bounds/timeouts/routes match C2.1; operator docs cover provider/security/persistence/single-replica semantics; `THIRD_PARTY_NOTICES` carries `oh-my-pi` attribution; the exact README YAML extraction test passes; no Devin token environment variable or historical plan edits were introduced. The sequential Chunk 11 commit is not yet created until the parent commits the implementation diff. Chunk 12 (C12.1–C12.6) completes final security, regression, and acceptance: focused union plus populated v4→v5 migration, `make check`, full race, and tagged smoke gates are green; the smoke harness completes local xAI device and Devin callback OAuth, exercises the exact 5-model×3-protocol×2-mode dispatch ledger, restart/reopen with cross-restart Responses continuation, admin/Web/CLI/worker projections, and a network-isolated populated prior-binary v4 backup restore with no in-place downgrade; final security fixes enforce a required, classified, strict-metadata/error-grammar Connect EndStream envelope and exact callback collision matching; the ultimate code and accounting gates are green after those fixes. C12.5 records the exact live-provider disposition: live Devin OAuth/inference was not attempted because operator approval was not provided, credential availability was not evaluated (no claim of unavailable), deterministic local fixtures are not live evidence, the live-only blocker is retained as evidence rather than an open C12.6 dependency, and C12.5 does not satisfy or close the four separately open `init.todo.md` blockers. The final review-fix record covers cache-read migration 007, CLI consumed-expiry, Web URL sweeper, strict Connect EndStream classification/metadata, callback matcher, xAI key-rotation fixture, and the CLEAN final split reviews; the ultimate gates are `make check` green, `go test -race ./...` 31 packages ok/2 no tests, tagged smoke race 2 packages ok, and `git diff --check` clean. The final sequential commit `test(provider): complete devin integration acceptance` is pending parent commit of the implementation diff; C12.6 is checked and no next implementation item remains.
>
> Current next item: none — all Devin tracker items C1.1–C12.6 are checked; the final sequential commit `test(provider): complete devin integration acceptance` is pending parent commit of the implementation diff (no commit is made by the tracker).
>
...
>
> Chunk 7 transport, mapper, generation, and usage evidence (2026-07-19): C7.1–C7.4 are complete. Connect requests use gzip protobuf flag `0x01`, big-endian framing, and exact streaming headers; raw/gzip response frames and JSON trailers are decoded under validated compressed, decompressed, total-stream, tool-argument, buffered-response, idle, caller, and optional earlier total-deadline bounds. A successful finalize requires a valid Connect EndStream envelope — a top-level JSON object that omits the error key on success; a present error key is parsed as a Connect error object and mapped to a typed provider.UpstreamError reusing the existing HTTP-status classification so routing can fail over before the first event (recognized codes: unavailable/internal → transient, resource_exhausted → rate-limit, unauthenticated → auth/relogin, permission_denied → permission, invalid_argument → validation), while structurally invalid errors (null, non-object, missing/empty/null/non-string code, invalid message or details field type, or any invalid details element — each details element must be a JSON object with a non-empty string type and a standard-base64 string value) are rejected as malformed; trailer metadata is exactly map[string][]string (arrays of UTF-8 strings) with scalar/null/object/non-string-array values rejected, each metadata string strictly validated against the raw JSON bytes before encoding/json replacement so raw invalid UTF-8 and unpaired or malformed UTF-16 surrogate escapes are rejected rather than silently U+FFFD-substituted, and valid metadata preserved when present — and finalize runs exactly once only after such a valid success envelope. Raw EOF is always truncation, never a clean success: a zero-byte fresh EOF and EOF after complete data frames both return a sanitized upstream protocol error because the required terminal EndStream envelope never arrived; partial headers/payloads, malformed trailers/protobuf, invalid flags, oversize data, gzip bombs, cancellation, and deadline failures remain deterministic and sanitized. The mapper preserves ordered text, reasoning/signature, tool-call, actual-model, …
>
...
>
> Chunk 9 implementation record (2026-07-19): C9.1–C9.4 are complete. One immutable two-provider runtime `CapabilityRegistry` composes the generation-complete xAI entry (Chunk 3 policy/generation/credentials plus Chunk 4 OAuth/refresh/discovery/usage) and the generation-complete Devin entry (Chunk 5 callback-PKCE `CredentialManager`, Chunk 6 bootstrap, Chunk 7 Connect stream/mapper/usage) over the single immutable static `ModelCatalog` constructed by C2.2; no static model entry is reconstructed, populated, or revalidated here. Startup performs static-to-runtime required-trio validation: every static entry whose `ResolvedModel.Provider` has a registered runtime must resolve the all-or-none `Policy` + `Generation` + `Credentials` trio, and startup fails only when a static entry references a missing required runtime provider/policy capability; optional Devin discoverer and usage fetcher remain absent (C8.3 OMIT disposition, C8.4 unavailable quota) without startup failure. Devin generation uses a pass-through `RequestPolicy` (no `x_search`, no `tool_choice` mutation) and a locked bounded Connect client with the C2.1 unary/frame/stream/size/idle/deadline bounds, custom-base trust, and per-request `GetUserJwt` bootstrap. Provider-aware defaults, readiness, and public listing resolve the configured default model from the static catalog, look up its runtime capabilities, and assess enabled usable accounts using `ResolvedModel.Provider` (never `OwnedBy`); `grok` is ready with a usable xAI account despite `OwnedBy=byos`, a configured Devin default is ready only with a usable Devin account and the required trio, and xAI backend-search readiness applies only to xAI defaults. End-to-end dispatch is exact for all five public names (`grok`, `grok-4.5`, `kimi-k2-7`, `glm-5-2`, `swe-1-6-slow`) across all three protocols (OpenAI Chat, OpenAI Responses, Anthropic Messages) and both response modes: `grok`/`grok-4.5` hit xAI only with mandatory `x_search` and `none -> auto`; the three Devin models hit Devin only without search, preserving absent/default, explicit `none`, `auto`, and selected-tool semantics; each provider wire payload is marshaled exactly once and no cross-provider endpoint is called. Provider-local retry, 429 `Retry-After`, cooldown, affinity, managed-Responses commitment, and per-response usage remain within the resolved provider; usage is charged to the selected account exactly once and managed Responses continuation cannot force a provider change. A shared `CooldownManager` `ReloginRequired` precedence fix corrected cooldown mutation so relogin-required cooldowns are not overwritten by lower-precedence classifications, with deterministic affinity fixtures added to cover wrong-provider preference fallback within the resolved provider. The runtime composition keeps a safe `internal/app`/`internal/provider` split: `internal/app` is the allowed composition root and alone imports and composes the concrete xAI/Devin generation, OAuth, discovery, and usage packages to wire the registry and inject the static catalog into account/model/usage/routing services, while shared routing/API and provider contracts/worker dispatch remain provider-neutral and provider-bound adapters remain isolated in xAI-specific model/usage files, with no provider import of app/routing. End-to-end dispatch is proven by a 30-row public matrix (five public names × three protocols × two response modes) plus focused tool-choice fixtures, an xAI private-encoder-counted-once proof, and a Devin unique top-level production `Marshal` callsite plus byte-exact protobuf/gzip/Connect frame proof. Provider-private request/response serialization is exact and covered by provider-private serialization tests asserting wire shape, field presence/order, and byte equivalence at the provider boundary. Readiness and credential usability share parity: both resolve the configured default model from the static catalog, look up its runtime capabilities, and assess enabled usable accounts by `ResolvedModel.Provider`, so a ready model always has a usable same-provider account and usability transitions mirror readiness. Persisted managed-Responses continuation, cooldown, per-response usage, and expired-credential failover survive restart and remain within the resolved provider. The unsafe exported/cross-package payload observer, dial-bypass, response-ID setter, and routing-clock seams introduced during this work were removed so tests exercise real construction, transport, and time paths without bypass hooks; harmless pre-existing package-private ID/time/encoder dependencies remain. No C10 Admin REST, Web, or CLI routes are introduced.
>
...
>
> Chunk 12 focused verification evidence (2026-07-20): `go test -count=1 ./internal/provider ./internal/store ./internal/crypto ./internal/accounts ./internal/oauth/xai ./internal/oauth/devin ./internal/models ./internal/usage ./internal/api/admin ./internal/app ./cmd/byos ./internal/devin/... ./internal/routing ./internal/xai ./internal/config` — 16 packages ok; `go test -count=1 -run '^TestProviderIdentityMigrationPreservesPopulatedV4$' ./internal/store` — PASS, proving the populated v4→v5 migration preserves account/oauth-session bytes, backfills `provider=xai` and `flow_type=device`, and leaves no partial schema. No focused contract was skipped.
>
> Chunk 12 full regression and race evidence (2026-07-20): `make check` (`go test ./...` + `go vet ./...` + gofmt gate) — exit 0, 31 packages ok, 2 no test files; `go test -race -count=1 ./...` — 31 packages ok, 2 no test files, zero race reports. The known go-oidc `RemoteKeySet` inflight race in `TestIdentityVerifierAndKeyRotation` was determined to be a test-synchronization issue (not production): the fix in `internal/oauth/xai/identity_test.go` synchronizes the async inflight clear and passes `count=100` focused and `count=10` full-package reruns plus the full race suite; no production code changed.
>
> Chunk 12 local smoke evidence (2026-07-20): `go test -tags smoke -race ./internal/app ./cmd/byos -timeout 600s` — 2 packages ok. The `smoke`-tagged harness (`internal/app/smoke_test.go`, `cmd/byos/smoke_test.go`) composes the real production component graph in-process with fake provider lifecycle/generation clients injected at approved network boundaries. It completes both xAI device and Devin callback OAuth logins into a real temp SQLite DB, serves real HTTP handlers, and asserts an exact call ledger across the five public models (`grok`, `grok-4.5`, `kimi-k2-7`, `glm-5-2`, `swe-1-6-slow`) × three protocols (Chat, Responses, Anthropic) × two stream modes (stream, non-stream) — 30 ordered entries with xAI always carrying `x_search` and Devin never carrying it and no cross-provider dispatch. Restart/reopen proves both accounts survive, post-restart xAI and Devin generation, cross-restart Responses `previous_response_id` continuation against the persisted session, and post-restart `/readyz`, `/v1/models`, admin accounts, and authenticated Web accounts rendering both providers. Management/CLI/workers are exercised: admin/Web/CLI provider selection, model/readiness projections, and worker dispatch. The populated prior-binary restore is offline: a populated v4 DB (migrations 001–004) is backed up, a separate copy is migrated forward, the pre-v5 commit `ecc6a18` is extracted via `git archive` (no worktree mutation) and built, and the prior binary opens the restored backup under a fail-closed proxy with the seeded account disabled and (when available) strace-verified no non-loopback `connect()`; the backup still has exactly 4 migrations, the representative session row is unchanged, and no in-place schema downgrade is attempted.
>
> Chunk 12 final security fixes and ultimate gates (2026-07-20): the Connect EndStream envelope is required and classified with strict metadata/error grammar — a successful finalize requires a valid non-empty top-level JSON object that omits the error key, raw EOF (fresh or after partial data frames) is truncation, valid Connect error objects are classified by code into a typed `provider.UpstreamError` before the first event so routing can fail over, and structurally invalid errors are rejected as malformed. Trailer metadata is parsed and strictly validated before the error branch for both success and error envelopes: raw JSON keys are UTF-8/surrogate-validated and checked against the Connect header-name grammar without `encoding/json` U+FFFD normalization, duplicate keys are rejected, and `error.details` elements are strictly typed/base64-validated; valid metadata is preserved on classified errors. Callback collision matching (`internal/app/callback_path_test.go` `validateCallbackPath`) rejects exact admin and Web route collisions, concrete dynamic-route matches, reserved subtrees, metacharacters, trailing slashes, and missing leading slashes, so the exact GET callback cannot shadow an admin/Web handler or bypass its session/CSRF wrappers. The ultimate code review and ultimate accounting review surfaced the EndStream metadata and callback collision findings; `C12StrictEndStreamFixer`, `C12CallbackMatcherFinalFixer`, and `C12XAIKeyRotationRaceFixer` resolved them; post-fix focused, full race, and smoke reruns are green, so the ultimate code and accounting gates are **CLEAN**.
>
> Chunk 12 live-provider disposition (C12.5, 2026-07-20): **BLOCKED** — live Devin OAuth/inference was not attempted because operator approval for live-provider execution was not provided. Credential availability was not evaluated and no claim of credential unavailability is made. Deterministic local fixtures are not live evidence. This live-only blocker is retained as evidence only and is not an open dependency for C12.6. C12.5 does not satisfy or close any of the four separately open `init.todo.md` blockers (`Initialize the Go module and pin dependencies`, `Perform live OAuth and x_search acceptance with two accounts`, `Perform live failover and restart acceptance`, `Complete final source and license review`); they remain separately unchecked under their own tracker. Automated/local acceptance (C12.1–C12.4) remains independently complete.
>
> Cache-read accounting completion evidence (2026-07-20): `provider.Usage.CacheReadTokens` is now threaded end-to-end through `routing.LocalUsageDelta` (both the `provider.Usage` fast path and the `response.completed`/`response.incomplete` JSON path reading `input_tokens_details.cached_tokens`), `usage.Delta`/`Counters`, `store.LocalUsageCounters`, and the real `LocalUsageRepository` upsert/scan. Migration `007_local_usage_cache_read.sql` adds `cache_read_tokens INTEGER NOT NULL DEFAULT 0 CHECK (>= 0)` to `local_usage_counters`, preserving existing v6 rows with default zero and keeping migration count at 7. The `usageRecorder`/`c9usageRecorder` adapters pass `CacheReadTokens` into `usage.Delta`, and Admin REST (`usageView.Local`), Web UI (`web.LocalUsage.CacheReadTokens` + `usage.html` "Cache read tokens" row), and the `webAccountAdapter` projection all expose it as a local proxy counter without treating it as billing quota. Evidence: `TestLocalUsageCacheReadMigrationPreservesPopulatedRows` (v6→v7 migration + accumulation + negative guard), `TestLocalUsageCountersPersistAcrossRestart` (nonzero cache-read survives reopen), `TestServiceRecordAndCountersThreadCacheReadTokens` (Record/Counters/Latest), `TestExecuteRecordsTerminalUsageExactlyOnce` and `TestStreamTerminalUsageRecordedExactlyOnce` (nonzero cache-read through both terminal paths), `TestUsageProjectionIncludesCacheReadTokens` (Admin JSON), `TestUsagePageRendersCacheReadTokens` (Web template), `TestC9Persistence_TerminalUsagePersistedOnceAcrossReopen` and `TestC9Persistence_ManagedResponsesContinuationPersistsPreferredAccountAcrossReopen` (end-to-end persistence/restart with cache-read=12 per dispatch). Gates: `make check` exit 0 (31 packages ok, 2 no test files); `go test -race -count=1 ./...` 31 packages ok, zero race reports; `go test -tags smoke -race ./internal/app ./cmd/byos -timeout 600s` 2 packages ok. No C12.6 checkbox or commit touched.
> Final review-fix record and ultimate gates (C12.6, 2026-07-20): the final acceptance review-fix pass resolved every remaining finding before the CLEAN final split. **Cache-read migration 007** — `provider.Usage.CacheReadTokens` is threaded end-to-end through `routing.LocalUsageDelta` (fast path and `response.completed`/`response.incomplete` `input_tokens_details.cached_tokens`), `usage.Delta`/`Counters`, `store.LocalUsageCounters`, and the real `LocalUsageRepository`; migration `007_local_usage_cache_read.sql` adds `cache_read_tokens INTEGER NOT NULL DEFAULT 0 CHECK (>= 0)` preserving v6 rows at default zero with migration count at 7; Admin REST, Web, and adapter projections expose it as a local proxy counter, never billing quota. **CLI consumed-expiry** — the CLI callback listener treats an already-consumed/expired OAuth session as terminal and stops polling rather than retrying or re-exchanging. **Web URL sweeper** — provider-selection and status rendering sweep only allowlisted, configured callback/origin URLs with no `Host`/forwarded-header construction and no Devin token environment variable. **Strict Connect EndStream classification/metadata** — a successful finalize requires a valid non-empty top-level JSON object omitting the error key; raw EOF (fresh or after complete data frames) is truncation; valid Connect error objects are classified by code into a typed `provider.UpstreamError` before the first event so routing can fail over; structurally invalid errors and malformed `map[string][]string` trailer metadata (invalid UTF-8, unpaired/malformed surrogate escapes, Connect header-name grammar violations) are rejected without `encoding/json` U+FFFD normalization; valid trailer metadata is preserved and finalize occurs exactly once only after a valid success envelope. **Callback matcher** — the configured GET callback matches exact method/path/provider/state/PKCE with single-use state and rejects wrong method/path/provider/error/code/state/replay without exchange, responses carry allowlisted lifecycle fields only, and concurrent OAuth session IDs cannot observe or affect one another. **xAI key-rotation fixture** — the go-oidc `RemoteKeySet` inflight race in `TestIdentityVerifierAndKeyRotation` was fixed as a test-synchronization issue (async inflight clear) with no production change, passing `count=100` focused and `count=10` full-package reruns plus the full race suite. **CLEAN final split reviews** — the final split acceptance reviews (Chunks 1–4, 5–8, 9–12) certified code, security, accounting, adversarial, and tracker disposition **CLEAN** with no open finding. Ultimate gates: `make check` exit 0 (31 packages ok, 2 no test files); `go test -race -count=1 ./...` 31 packages ok, 2 no test files, zero race reports; `go test -tags smoke -race ./internal/app ./cmd/byos -timeout 600s` 2 packages ok; `git diff --check` clean. C12.5's blocked live disposition and the four separately open `init.todo.md` blockers (`Initialize the Go module and pin dependencies`, `Perform live OAuth and x_search acceptance with two accounts`, `Perform live failover and restart acceptance`, `Complete final source and license review`) remain preserved and unchecked; this tracker neither superseded nor closed them. The final sequential commit `test(provider): complete devin integration acceptance` is pending parent commit of the implementation diff; no commit is made by the tracker.
...
- [x] **C7.2 — Map Devin stream responses to neutral events**
  ...
  - Work: map text, thinking/signature, tool-call start/delta/stop, actual model, explicit/inferred stop, usage, and message completion. Handle incremental versus cumulative tool arguments without duplication. A successful finalize requires a valid Connect EndStream envelope: a top-level JSON object that omits the error key on success; a present error key is parsed as a Connect error object and mapped to a typed provider.UpstreamError reusing the existing HTTP-status classification so routing can fail over before the first event (recognized codes: unavailable/internal → transient, resource_exhausted → rate-limit, unauthenticated → auth/relogin, permission_denied → permission, invalid_argument → validation), while structurally invalid errors (null, non-object, missing/empty/null/non-string code, invalid message or details field type, or any invalid details element — each details element must be a JSON object with a non-empty string type and a standard-base64 string value) are rejected as malformed; trailer metadata is exactly map[string][]string (arrays of UTF-8 strings) with scalar/null/object/non-string-array values rejected, each metadata string strictly validated against the raw JSON bytes before encoding/json replacement so raw invalid UTF-8 and unpaired or malformed UTF-16 surrogate escapes are rejected rather than silently U+FFFD-substituted, and valid metadata preserved when present; finalize runs exactly once only after such a valid success envelope. Raw EOF is always truncation, never a clean success: a zero-byte fresh EOF and EOF after zero or more complete data frames both return a sanitized upstream protocol error because the required terminal EndStream envelope never arrived. EOF after 1–4 header bytes or before the declared payload completes is likewise truncation. Reject malformed end-stream JSON as an intentional hardening.
  - Observable DoD: multi-frame output preserves event order; explicit stop wins and mapper finalization runs exactly once only after a valid success EndStream envelope; raw EOF (empty stream or after any complete-frame sequence) is truncation, not success; a top-level JSON object is required, success omits the error key, valid Connect error objects are classified by code into typed provider.UpstreamError so routing can fail over before the first event, structurally invalid errors and malformed map[string][]string metadata are rejected, and valid trailer metadata is preserved; partial header/payload returns a sanitized truncation error; no event is emitted twice.
  - Verification: raw/gzip multi-frame, cumulative/delta, explicit/inferred stop, raw-EOF truncation (empty stream and EOF after complete frames), EndStream envelope matrix (top-level object required, success omits the error key, valid Connect error objects classified by code into typed provider.UpstreamError, structurally invalid errors and malformed map[string][]string metadata rejected, valid metadata preserved), partial-header, partial-payload, malformed-trailer, and stop-reason fixtures.
...
- [x] **C9.1 — Compose the immutable two-provider runtime capability registry**
...
- [x] **C9.2 — Make readiness and public catalog follow the default model owner**
  - Chunk owner: **Chunk 9**.
  - Depends on: C9.1.
...
- [x] **C9.3 — Prove end-to-end provider dispatch for all public protocols**
  - Chunk owner: **Chunk 9**.
  - Depends on: C9.2.
...
- [x] **C9.4 — Prove provider-local retry, affinity, cooldown, and accounting end to end**
  - Chunk owner: **Chunk 9**.
  - Depends on: C9.3.
...
- [x] **C9.5 — Review and commit Chunk 9**
  - Chunk owner: **Chunk 9**.
  - Depends on: C9.4.
...
- [x] **C12.1 — Run focused provider and migration verification**
...
- [x] **C12.2 — Run full automated regression and race gates**
...
- [x] **C12.3 — Smoke test both providers with local fixtures and restart**
  - Chunk owner: **Chunk 12**.
  - Depends on: C12.2.
...
- [x] **C12.4 — Perform the final security and scope review**
  - Chunk owner: **Chunk 12**.
  - Depends on: C12.3.
...
- [x] **C12.5 — Record live-provider acceptance honestly**
  - Chunk owner: **Chunk 12**.
  - Depends on: C12.4.
...
- [x] **C12.6 — Review and commit final acceptance**
  - Chunk owner: **Chunk 12**.
  - Depends on: C12.5.
...
9. Are Connect frames, decompression, total stream data, tool arguments, and non-stream buffering bounded by the exact C2.1 settings; is total stream lifetime caller-context-driven with the adapter deadline disabled by default; does a successful finalize require a valid Connect EndStream envelope (top-level JSON object, error key omitted on success, valid Connect error objects classified by code into typed provider.UpstreamError so routing can fail over before the first event, structurally invalid errors and malformed map[string][]string metadata rejected, valid trailer metadata preserved) with finalize exactly once only after a valid success envelope; and do fixtures prove raw EOF (zero bytes at a fresh header after zero or more complete frames) is truncation while partial header/payload also truncates?
