...
- Devin model discovery is optional and, if implemented, may only intersect the fixed three-model set. Devin quota/statistics, capacity preflight, analytics, credit/ACU reporting, and usage-submission RPCs are out of scope.
...
- [ ] **C8.3 — Optionally implement safely bounded Devin model discovery**
  - Chunk owner: **Chunk 8**.
  - Depends on: C8.2.
  - Files: Devin optional discoverer and tests, only if retained after implementation review.
  - Work: call source-supported `GetCliModelConfigs` with normalized session token and default runtime base, without `GetUserJwt` or custom chat base; bound response/decompression/count/string sizes; ignore disabled/empty records and untrusted pricing/provider/default/capacity metadata; trim/deduplicate IDs; intersect results with exactly the three static Devin models.
  - Observable DoD: discovery can mark only the three fixed models supported/unsupported/unknown and can never add a fourth public/routable model or change ownership; failure/stale state is sanitized and does not disable static unknown-capability fallback unexpectedly.
  - Verification: disabled/empty/duplicate/oversized/incidental-model/failure fixtures. If omitted, record the explicit decision in the implementation review and keep all call sites optional; do not leave a stub.
...
- [ ] **C12.5 — Record live-provider acceptance honestly**
  - Chunk owner: **Chunk 12**.
  - Depends on: C12.4.
  - Work: if real Devin credentials and operator approval are available, perform scrubbed live Devin login/inference for owned models and both stream modes. If unavailable, record the exact missing credential/approval as live-only blocked evidence without weakening deterministic local acceptance or marking the scenario passed. This item is Devin-specific and cannot satisfy or close any historical `init.todo.md` task.
  - Observable DoD: no fake fixture is represented as live; no real secret or prompt is retained; automated/local acceptance remains independently complete; the four historical blockers remain separately unchecked unless completed under their own tracker.
  - Verification: sanitized Devin operator record or explicit Devin live-only blocker, plus explicit confirmation that C12.5 did not close `Initialize the Go module and pin dependencies`, `Perform live OAuth and x_search acceptance with two accounts`, `Perform live failover and restart acceptance`, or `Complete final source and license review` in `init.todo.md`.

- [ ] **C12.6 — Review and commit final acceptance**
  - Chunk owner: **Chunk 12**.
  - Depends on: C12.5.
  - Observable DoD: all preceding deterministic and non-live Devin tracker items are checked; only an explicitly identified Devin credential/approval-blocked live-only scenario may remain unchecked with honest evidence. The architecture plan, shipped code, tests, routes, config, docs, attribution, and security evidence agree; no historical planning artifact changed and none of its four open blockers was superseded or closed.
...
11. Can optional discovery only intersect the fixed three-model set, and is unsupported quota explicitly unavailable without blocking inference?
