...
func TestClientAuth(t *testing.T) {
	...
	for _, test := range []struct {
		name, header string
		status       int
	}{{"missing", "", 401}, {"malformed", "Basic x", 401}, {"unknown", "Bearer byoo_unknown", 401}, {"valid", "Bearer " + created.Plaintext, 204}} {
...
func TestAdminAuthIsSeparate(t *testing.T) {
	...
	for _, header := range []string{"Bearer byoo_client", "Bearer wrong", ""} {
