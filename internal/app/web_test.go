...
func (adapterAPIKeys) Create(context.Context, string) (accounts.CreatedAPIKey, error) {
	return accounts.CreatedAPIKey{Key: store.APIKey{ID: "key_new", Prefix: "byoo_new", Label: "New"}, Plaintext: "byoo_one_time_secret"}, nil
}
...
func TestWebAdaptersProjectOnlySafeManagementData(t *testing.T) {
	...
	keyAdapter := &webAPIKeyAdapter{service: adapterAPIKeys{values: []store.APIKey{{ID: "key_safe", Prefix: "byoo_prefix", Label: "Client", CreatedAt: now}}}}
	...
	if err != nil || created.Plaintext != "byoo_one_time_secret" {
...
func TestRuntimeMountsWebHandler(t *testing.T) {
	...
	t.Setenv("BYOO_MASTER_KEY", base64.StdEncoding.EncodeToString(master))
	t.Setenv("BYOO_ADMIN_PASSWORD", "runtime-admin-password")
	t.Setenv("BYOO_ADMIN_API_KEY", "runtime-admin-api-key")
