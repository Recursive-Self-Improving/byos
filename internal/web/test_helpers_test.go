...
func newWebFixture(t *testing.T, configure ...func(*Options)) *webFixture {
	...
	usage := &fakeUsageService{values: []AccountUsage{{Provider: ProviderXAI, AccountID: "acct_test", AccountLabel: "Primary account", QuotaAvailable: true, CanRefresh: true, Monthly: UsagePeriod{Available: true, Used: 25, Limit: &limit, Percent: &monthly, Unit: "credits"}, Weekly: UsagePeriod{Available: true, Used: 40, Limit: &limit, Percent: &weekly, Unit: "credits"}, Local: LocalUsage{Requests: 10, InputTokens: 200, OutputTokens: 80}, FetchedAt: &fetched}}}
