...
func TestOAuthAuthorizationAndCompletionSurviveRestart(t *testing.T) {
	...
	if err := repository.Complete(ctx, session.State, "acct_too_early", now); !errors.Is(err, sql.ErrNoRows) {
	...
	if err := repository.Authorize(ctx, session.State, authorization, now); err != nil {
		t.Fatal(err)
	}
	if _, err := repository.GetPending(ctx, session.State, now); !errors.Is(err, sql.ErrNoRows) {
	...
	resumable, err := repository.ListResumable(ctx, now)
	...
	if err := repository.Complete(ctx, session.State, "acct_persisted", now.Add(time.Second)); err != nil {
		t.Fatal(err)
	}
	completed, err := repository.Get(ctx, session.State)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err := repository.Complete(ctx, session.State, "acct_other", now.Add(2*time.Second)); !errors.Is(err, sql.ErrNoRows) {
		t.Fatalf("terminal completion mutated: %v", err)
	}
	if values, err := repository.ListResumable(ctx, now); err != nil || len(values) != 0 {
