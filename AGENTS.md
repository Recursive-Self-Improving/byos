...
- OpenAI Responses `function_call_output.status` does not allow `"failed"`; Anthropic `tool_result.is_error` must not be translated to that value. Anthropic translation also needs to preserve `tool_choice.disable_parallel_tool_use` and adaptive thinking semantics.
