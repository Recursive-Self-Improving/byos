...
`byoo` (bring-your-own-oauth) is a single-process Go service that exposes OpenAI- and Anthropic-compatible HTTP APIs backed by xAI accounts authenticated through the xAI device flow. It includes an administrator Web UI for connecting accounts, checking model and usage state, and issuing downstream API keys.
...
flowchart LR
    ...
    subgraph S[byoo - single process]
...
| --- | --- |
| `BYOO_MASTER_KEY` | Exactly 32 random bytes encoded with standard base64. Back it up; changing it makes encrypted persisted data unreadable. |
| `BYOO_ADMIN_PASSWORD` | Password for `/admin/login`. |
| `BYOO_ADMIN_API_KEY` | Bearer token for `/admin/api/v1/*`; separate from downstream client keys. |

Each secret may instead be supplied through a file by setting the corresponding `_FILE` variable, such as `BYOO_MASTER_KEY_FILE`.
...
```sh
export BYOO_VERSION=local
export BYOO_COMMIT="$(git rev-parse --short HEAD)"
export BYOO_BUILD_DATE="$(date -u +%Y-%m-%dT%H:%M:%SZ)"
export BYOO_MASTER_KEY="$(openssl rand -base64 32)"
export BYOO_ADMIN_PASSWORD="$(openssl rand -base64 24)"
export BYOO_ADMIN_API_KEY="$(openssl rand -hex 32)"
```
...
- stores SQLite data in the named `byoo-data` volume mounted at `/data`;
...
2. Sign in with `BYOO_ADMIN_PASSWORD`.
...
curl http://127.0.0.1:8080/v1/chat/completions \
  -H "Authorization: Bearer $BYOO_CLIENT_API_KEY" \
...
```sh
# Follow logs
docker compose logs -f byoo

# Restart without deleting persisted data
docker compose restart byoo
...
```text
BYOO_MASTER_KEY=<base64-encoded 32-byte key>
BYOO_ADMIN_PASSWORD=<strong administrator password>
BYOO_ADMIN_API_KEY=<strong administrator bearer token>
```
...
Keep `BYOO_MASTER_KEY` stable for the lifetime of the volume and back it up outside Railway. The Docker build metadata arguments are optional on Railway because `Dockerfile.railway` provides safe defaults.
...
export BYOO_ADMIN_PASSWORD="$(openssl rand -base64 24)"
export BYOO_ADMIN_API_KEY="$(openssl rand -hex 32)"
go run ./cmd/byoo serve
```

Defaults are `127.0.0.1:8080` and `./data`. Optional CLI overrides:

```text
byoo serve [--config path] [--listen address] [--data-dir path]
byoo login [--config path] [--listen address] [--data-dir path]
byoo version
```
...
- Keep `/data` persistent and private. Back it up, and retain `BYOO_MASTER_KEY` securely in a separate secret store.
